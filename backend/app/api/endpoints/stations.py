        ...
        "latest_prices": [
            {
                ...
                "price_flagged": getattr(p, "price_flagged", False),
                "is_county_cheapest": getattr(p, "is_county_cheapest", False),
            }
            for p in latest
        ],
...
async def get_price_history(
    ...
    return PriceHistoryOut(
        ...
        fuel_type=fuel,
        history=[
            PriceHistoryPoint(recorded_at=r.recorded_at, price_pence=r.price_pence)
            for r in records
        ],
    )
...
async def _get_latest_prices(
    ...
    result = await db.execute(stmt)
    records = result.scalars().all()
    # Get station county for cheapest comparison
    county_res = await db.execute(
    ...
    return [
        StationLatestPrices(
            ...
            price_flagged=r.price_flagged or False,
            is_county_cheapest=(
                not (r.price_flagged or False)
                and county_cheapest.get(r.fuel_type) == r.price_pence
            ),
        )
        for r in records
    ]
...
async def get_price_changes(
    ...
    old_result = await db.execute(
        old_sql, {"station_id": station_id, "day_ago": day_ago}
    )
